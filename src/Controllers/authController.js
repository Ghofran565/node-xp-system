...
export const register = catchAsync(async (req, res, next) => {
	...
	if (!passwordRegex.test(password))
		return next(new HandleError('Invalid password format.', 400));
	if (!username || username.trim().length < 3)
		return next(new HandleError('Invalid username.', 400));
...
export const verifyEmail = catchAsync(async (req, res, next) => {
	...
	const verificationResult = await import('../Utils/notifier.js').then((m) =>
		m.verifyEmailCode(email, code)
);

log(verificationResult.authorized).warn().white()
log("test").warn().white()

	if (!verificationResult.authorized)
		return next(new HandleError(verificationResult.message, 401));
	...
	if (!player)
		return next(new HandleError('User not found or already verified.', 404));
...
export const login = catchAsync(async (req, res, next) => {
	...
	if (totalCount === 0 || !bcryptjs.compareSync(password, player.password)){
...
export const resetPassword = catchAsync(async (req, res, next) => {
	...
	if (!emailRegex.test(email))
		return next(new HandleError('Invalid email format.', 400));
	if (!verificationCodeRegex.test(code))
		return next(new HandleError('Invalid verification code.', 400));
	if (!passwordRegex.test(password))
		return next(new HandleError('Invalid password format.', 400));

	const verificationResult = await import('../Utils/notifier.js').then((m) =>
		m.verifyEmailCode(email, code, 'reset')
	);
	if (!verificationResult.authorized)
		return next(new HandleError(verificationResult.message, 401));
